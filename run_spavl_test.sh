#!/bin/bash
# SIA Evaluation on SPA-VL dataset - Test run (5 samples)
...
echo "Running SIA test evaluation on SPA-VL dataset (5 samples)..."
...
uv run python eval_vlguard.py \
    ...
    --output-file results/spavl_sia_test.json \
    --temperature 0.2 \
    --max-new-tokens 1024 \
    --limit 5

echo ""
echo "Test complete! Check results/spavl_sia_test.json"
